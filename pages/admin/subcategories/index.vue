    ...
    >
      ...
      <table class="w-full">
        <thead class="rounded-10" style="background: #e0e0e0">
          <tr class="text-$blue_d">
            ...
            <td class="p-2">Добавлен</td>
          </tr>
        </thead>
        <tbody>
          <tr
            v-for="category in sub_categoryes"
            :key="category.id"
            class="mt-1"
          >
            <td class="p-2">
      ...
      >
        <button
          ...
          @click="
            $router.push(`/admin/subcategories/${sub_categoryes[findSome].id}`)
          "
        >
          Изменить
        </button>
...
export default {
  ...
  computed: {
    findSome() {
      let result = false
      for (let i = 0; i < this.sub_categoryes.length; i++)
        if (result && this.sub_categoryes[i].active) {
          result = false
          break
        } else if (this.sub_categoryes[i].active) result = i
      return result
    },
    checkAll() {
      let result = true
      for (let i = 0; i < this.sub_categoryes.length; i++)
        if (!this.sub_categoryes[i].active) {
          result = false
          break
        }
      return result
    ...
    category_id(newData) {
      if (newData && newData.id)
        this.$axios.get("category/" + newData.id).then((res) => {
          this.all = false
          this.sub_categoryes = res.data.category.map((x) => {
            return {
              ...x,
              active: false,
            }
  ...
  methods: {
    deleteCat() {
      for (let i = 0; i < this.sub_categoryes.length; i++)
        if (this.sub_categoryes[i].active) {
          this.$axios.delete("/admin/category/" + this.sub_categoryes[i])
          this.sub_categoryes.splice(i, 1)
        }
    },
  },
}
